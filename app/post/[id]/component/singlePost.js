...
const Post = ({ showPost }) => {
  ...
  useEffect(() => {
    ...
    setlikedPost({ [showPost._id]: showPost.isLiked });
    setPost(showPost);
  }, [showPost]);

  const handleFollow = async (targetID) => {
    const follow = await followUser(targetID);
    setFollowingMap((prev) => ({ ...prev, [targetID]: follow.following }));
  };
  ...
  return (
    <div className="min-h-[88vh] bg-stone-50 flex justify-center items-start pt-54 md:pt-34 px-4">
      <article className="bg-white w-full max-w-2xl rounded-2xl border border-stone-200 p-4 px-2 sm:p-5 shadow-sm">
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="flex items-center pl-0.5 gap-1.5 flex-wrap">
                  ...
                  {post.verified && (
                    <span className="bg-amber-400 text-white text-[9px] font-bold px-2 py-0.5 rounded-full tracking-wide shrink-0">
                      PRO
                    </span>
                  )}
                </div>
